...
- Swap space
	- swap space는 하드디스크의 일부분으로 컴퓨터의 가상 메모리의 일부이다.
	- swap space는 RAM에서 프로세스가 돌고 있고 다른 더 중요한 프로세스가 쓰기에 RAM이 충분하지 않을 경우, OS가 일부러 원래 RAM을 사용하고 있던 프로세스를 swap space로 옮겨놓고 더 중요한 다른 프로세스에게 RAM을 할당시켜준다.
		- 다른 프로세스가 RAM을 다 사용하면 OS는 swap space에 있던 프로세스를 다시 RAM으로 가져와 사용할 수 있게 만들어준다.
>ref : https://help.ubuntu.com/community/SwapFaq#:~:text=your%20own%20opinion.-,What%20is%20swap%3F,pages%20that%20are%20temporarily%20inactive
